...
__global__ void  sumChannels(const double *Params, const float *data, 
  ...
  while (tid0<NT){
      ...
      // loop through templates and sizes to find max
      for (t=0;t<Nrank;t++){                             
          for(k=0; k<nsizes; k++)
              a[k] = 0.;
...
//////////////////////////////////////////////////////////////////////////////////////////
__global__ void  spikePC(const double *Params, const float *data, 
	float *feat, const int *st, const int *iC, const float *wPCA){
    
  int indx, nt0, totThreads, tid,k, t0, NT, NchanNear,j,iChan, Nrank, Z;
  float a;
  ...
  // each thread takes a filter and a near channel
  k = threadIdx.x;
  
  indx = blockIdx.x;
  Z    = st[1+4*indx];
  t0   = st[0+4*indx];
  iChan = iC[threadIdx.y + NchanNear * Z]; // Z is the best channel of this spike
...
__global__ void	max1D(const double *Params, const float *data, float *conv_sig){
    ...
    while (tid0<NT-Nthreads-nt0+1){
        ...
        if (y>spkTh*spkTh)
            conv_sig[tid0 + nt0min  + tid + NT*bid]   = y;
                 ...
                 int nrhs, mxArray const *prhs[])
{
  ...
  mxGPUArray const  *W,  *wPCA, *data, *iC, *iC2, *dist, *v2, *dist2;
  const float     *d_W, *d_data, *d_dist, *d_v2, *d_dist2, *d_wPCA;
  ...
  wPCA             = mxGPUCreateFromMxArray(prhs[8]);
  d_wPCA        	= (float const *)(mxGPUGetDataReadOnly(wPCA));
  
  /* allocate new GPU variables*/  
  float *d_dout, *d_dfilt, *d_feat, *d_dmax, *d_cF;
  int *d_kkmax;
  int *d_st,  *d_counter;
  ...
  cudaMalloc(&d_cF,     maxFR * sizeof(float));  
  ...
  cudaMemset(d_cF,      0, maxFR *   sizeof(float));  
  ...
  maxChannels<<<tpC,Nthreads>>>(d_Params, d_dout, d_dmax, d_iC, d_iC2, d_dist2, d_kkmax,
          d_dfilt,  d_st, d_counter, d_cF);
  
  cudaMemcpy(counter,     d_counter, sizeof(int), cudaMemcpyDeviceToHost);
 
   // template products for each spike
  unsigned int minSize=1;
  minSize = min(maxFR, counter[0]);  
  dim3 ts(Nrank, NchanNear);
  cudaMalloc(&d_feat,     NchanNear*Nrank * minSize * sizeof(float));
  cudaMemset(d_feat,     0, NchanNear*Nrank * minSize * sizeof(float));  
  spikePC<<<minSize, ts>>>(d_Params, d_data, d_feat, d_st, d_iC, d_wPCA);
  ...
  float *d_cF2;
  mxGPUArray *cF2;
  const mwSize dcf[] 	= {1, minSize};
  cF2 		= mxGPUCreateGPUArray(2, dcf, mxSINGLE_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
  d_cF2 		= (float *)(mxGPUGetData(cF2));   
  cudaMemcpy(d_cF2, d_cF, minSize * sizeof(float),   cudaMemcpyDeviceToDevice);
  
  float *d_cFall;
  mxGPUArray *cFall;
  const mwSize dcfall[] 	= {Nrank, NchanNear, minSize};
  cFall 		= mxGPUCreateGPUArray(3, dcfall, mxSINGLE_CLASS, mxREAL, MX_GPU_DO_NOT_INITIALIZE);
  d_cFall 		= (float *)(mxGPUGetData(cFall));
  cudaMemcpy(d_cFall, d_feat, NchanNear*Nrank*minSize * sizeof(float),   cudaMemcpyDeviceToDevice);
...
// dWU stays a GPU array
  ...
  plhs[4] 	= mxGPUCreateMxArrayOnGPU(cFall);
